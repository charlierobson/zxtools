...
int main(int argc,char *argv[])
{
    ...
    int iInputLineNum = 0;
    int iPassNum = 1;
    static unsigned char buf[16384],lcasebuf[1024],outbuf[2048];
    ...
    do {
        ...
        while( fgets(buf+1,sizeof(buf)-1,fIn) != NULL ) {
            ...
            long machineCodeLen = 0;
            if(buf[1]=='!') {
                int i;
                machineCode = fopen(buf+2,"r");
                if (machineCode == NULL) {
    ...
    */
    ...
    sysvarbuf[31]=55;
    sysvarbuf[32]=sysvarbuf[3];  /* NXTLIN low  - Address of next program line to be executed */
    sysvarbuf[33]=sysvarbuf[4];  /* NXTLIN high */
